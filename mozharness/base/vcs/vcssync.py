...
class VCSSyncScript(VCSScript):
    ...
    def notify(self, message=None, fatal=False):
        ...
        if error_contents:
            if len(error_contents) > 102400:
                text += '\n*** Message below has been truncated: it was %s characters, and has been reduced to 102400 characters:\n' % len(error_contents)
            text += '\n%s\n\n' % error_contents[0:102400] # limit message to 100KB in size (large emails fail to send)
        if not text:
            subject += " <EOM>"
